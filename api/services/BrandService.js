const BrandService = {
    get: async (options) => {
        ...
        let records = await Brand.findOne(options);
        return records;
    },
    add: async (options) => {
    ...
    let newObj = await Brand.create(options)
    ...
    edit: async (query, params) => {
        ...
        for (let key in Brand.attributes) {
        ...
        let editObj = await Brand.update(query, options).fetch();
    ...
    del: (options, cb) => {
        ...
        Brand.destroy(options).exec((error, deletedRecords) => {
          if (error) {
    ...
    find: async (where, limit, skip, sort) => {
        ...
        let sujects = await Brand.find({ where: where, limit: limit, skip: skip, sort: sort })
          return sujects;
    },
    count: async (where) => {
        where = (typeof where === 'object') ? where : {};
        let totalTax = await Brand.count(where);
        return totalTax;
    }
}
