...
class ListIndexProductBackendEKP {
	...
	initSweetAlert(id) {
		swal({
			...
			if (value) {
				Cloud.trashProduct.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
	...
	initDataTable() {
		...
		window.dataTable = this.tableObj.DataTable({
			...
			"processing": true,
			"serverSide": true,
			"ajax": `/api/v1/backend/product/search?status=${params.status}`,
