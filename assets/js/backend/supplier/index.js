class IndexListSupplierBackendEKP extends BaseBackendEKP {
	constructor() {
		super();
		this.initialize();
	}
	...
	initialize() {
		...
		this.list = new ListIndexSupplierBackendEKP();
		this.form = new FormIndexSupplierBackendEKP();
	}
}


class ListIndexSupplierBackendEKP {
	constructor(opts) {
		_.extend(this, opts);

		this.tblId = 'tblSupplier';
	...
	initSweetAlert(id) {
		swal({
			...
			if (value) {
				Cloud.trashSupplier.with({ ids: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
					if (err) {
						console.log(err);
						return;
	...
	handleItemActions() {
      ...
      _this.tableObj.on('click', '.edit-row', function (e) {
        ...
        //get AJAX data
        Cloud.getSupplier.with({ id: id }).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
          if (err) {
            console.log(err);
            return;
          }
...
class FormIndexSupplierBackendEKP {
    constructor(opts) {
      _.extend(this, opts);
      this.formId = 'formSupplier';
      ...
      if (this.formObj.length) {
        ...
        this.messages = {
          headlineAdd: 'Thêm nhà cung cấp',
          headlineUpdate: 'Cập nhật nhà cung cấp',
          addSuccess: 'Thêm mới thành công',
          editSuccess: 'Cập nhật thành công.',
          error: 'Có lỗi xảy ra',
      ...
      _this.formObj.formValidation({
        ...
        .on('success.form.fv', function (e) {
          ...
          if (manner === 'edit') {
            ...
            Cloud.editSupplier.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
              if(err){
            ...
            Cloud.addSupplier.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
              if(err){
                _this.alert.removeClass('d-none').addClass("alert-fill-danger").html(_this.messages.error);
                setTimeout(function(){
