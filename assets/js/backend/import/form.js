class IndexFormImportBackendEKP extends BaseBackendEKP {
	...
	initialize() {
		...
		this.form = new FormIndexImportBackendEKP();
	}
}
    ...
    initValidation() {
      ...
      _this.formObj.formValidation({
        ...
        .on('success.form.fv', function (e) {
            ...
            if (_this.coutItemTuition > 0) {
              ...
              for (let i = 0; i < _this.coutItemTuition; i++) {
                if (item.name == 'titleSlotItem' + i) {
                  objImport.id = i;
                  objImport.product = item.value;
                  slotItems.push(objImport);
                }
          ...
          Cloud.addImport.with(tmpData).protocol('jQuery').exec((err, responseBody, responseObjLikeJqXHR) => {
            if(err){
